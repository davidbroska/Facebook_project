...
source(list.files(pattern="1_Setup.R", recursive=T))


# not used anymore
# use 3_Running_rca instead
...
rescale_cat = function(x){
  ...
  x[,1:length(x)] <- purrr::map(x[,1:length(x)], as.factor)
...
vars = c("ResponseId", BinaryDvs)

# Names of relevant variables
labels = c("ID", BinaryDvs)
